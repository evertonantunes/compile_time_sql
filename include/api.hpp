...
namespace sql
{
    ...
    template< typename ... T>
    constexpr auto concatenate( const T &...args )
    {
        return ((args) + ...);
    }
...
//                const constexpr auto x = SelectFromWHERE::to_string();
//                std::cout << "sql: " << &x.data[0] << std::endl;
...
//            }
            ...
            static constexpr auto to_string( )
            {
                return "SELECT "_s + ((T::to_string() + ", "_s) + ... + " 0 as ignored "_s) + " "_s;
            }

...
namespace database
{
    ...
    namespace tables
    {
        ...
        struct UT: public Table<decltype("T_UT"_s)>
        {
            ...
            static constexpr Column<UT, decltype("value"_s)  , std::string_view , Flags<not_null>                           > value     = {};
        ...
        struct FL: public Table<decltype("T_FL"_s)>
        {
            ...
            static constexpr Column<FL, decltype("value"_s)  , double        , Flags<not_null>                              > value = {};
        ...
        struct DT: public Table<decltype("T_DT"_s)>
        {
            ...
            static constexpr Column<DT, decltype("value"_s)  , std::string_view , Flags<not_null>                           > value     = {};
        ...
        struct LT: public Table<decltype("T_LT"_s)>
        {
            ...
            static constexpr Column<LT, decltype("value"_s)  , std::string_view , Flags<not_null>                           > value     = {};
...
namespace
{
    ...
    void load_vr(const std::ptrdiff_t dataset_id, const std::ptrdiff_t tag, const nlohmann::json &data)
    {
        ...
        if constexpr (std::is_same_v<T, PN>)
        {
            ...
            {
                if (item.find("Alphabetic") != end(item))
                {
                    const nlohmann::json &alphabetic = item["Alphabetic"];
                    if (!alphabetic.is_string())
                    {
                        continue;
                    }
                    database::insert_into<T>(  T::tag = tag
                                             , T::dataset = dataset_id
                                             , T::value = alphabetic.get<type_t>() );
                }
            }
        }
    ...
    bool load( const std::ptrdiff_t dataset_id, const nlohmann::json &data )
    {
        using namespace database::tables;
        ...
        for ( const auto &item : data.items())
        {
            ...
            if (item.value().find("vr") == end(item.value()))
            {
                return false;
            }
